...
json_payload="{\"filter\": {\"interface\": \"$interface\", \"namespace\": \"$namespace\", \"enable_ipv4\": $enable_ipv4, \"enable_ipv6\": $enable_ipv6, \"enable_icmp\": $enable_icmp, \"enable_tcp\": $enable_tcp, \"enable_udp\": $enable_udp, \"ipv4_range\": [\"$ipv4_range\"], \"ipv4_mask\": \"$ipv4_mask\"}, \"delay\": $delay, \"jitter\": $jitter, \"drop_horizon\": $drop_horizon}"

# query the gRPC-Service
sudo docker run --network host fullstorydev/grpcurl -plaintext -d "$json_payload" $pod_ip:8080 networkfilter.NetworkFilter/EnableDelayGen
