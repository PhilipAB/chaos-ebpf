...
# disable bitflip generator
json_payload="{\"interface\": \"$interface\", \"namespace\": \"$namespace\", \"traffic_direction\": $traffic_direction}"
sudo docker run --network host fullstorydev/grpcurl -plaintext -d "$json_payload" $pod_ip:8080 networkfilter.NetworkFilter/DisableBitflipGen
